...
# euclidean loss calculation can be found in line 507 to line 522. --RESOLVED
...
# baseVAE.setup_anndata(x_adata, layer="counts")
# # vae1 = baseVAE(x_adata, n_latent=LATENT_DIM, latent_distribution='normal')
vae3.train(max_epochs=NUM_EPOCHS)
zbar = vae3.get_latent_representation()
plt.figure(figsize=(10,10))
plt.scatter(z_prime[:, 0], z_prime[:, 1], label = "2D latent representation: scRNA 500 genes")
plt.scatter(zbar[:, 0], zbar[:, 1], label = "2D latent representation: shared ST 500 genes")
plt.legend(loc = 'upper right')
plt.show()
