...
from models.sedrModels.src.SEDRVAE import SEDRVAE
...
def vae_loss(mu, logvar, n_nodes):
    # if mask is not None:
    #     preds = preds * mask
    #     labels = labels * mask
...
class SEDRVAE_Trainer:
    '''
    Edits made to this class from the original code
    '''
    def __init__(self, node_X, params, version):
        self.params = params
        ...
        if version == 1:
            self.model = SEDRVAE(self.params.cell_feat_dim, self.params).to(self.device)
        
        self.optimizer = torch.optim.Adam(params=list(self.model.parameters()),
                                          lr=self.params.lr, weight_decay=self.params.gcn_decay)
    ...
    def train_one_episode_without_dec(self):
        ...
        start_time = time.time()
        self.optimizer.zero_grad()
        latent_z, mu, logvar, de_feat, _= self.model(self.node_X)
        
        loss_kl = vae_loss(mu=mu, logvar=logvar, n_nodes=self.params.cell_num)
        loss_rec = reconstruction_loss(de_feat, self.node_X)
        loss = loss_rec
        # loss = self.params.feat_w * loss_rec + self.params.kl_w * loss_kl
        loss.backward()
        self.optimizer.step()
    ...
    def process(self):
        self.model.eval()
        latent_z, mu, logvar, de_feat, q = self.model(self.node_X)
        latent_z = latent_z.data.cpu().numpy()
        q = q.data.cpu().numpy()
        return latent_z, mu, logvar, q
