...
class SEDR_V2(nn.Module):
    def __init__(self, input_dim, params):
        super(SEDR_V2, self).__init__()
        self.alpha = 1.0
        self.latent_dim = params.gcn_hidden2 + params.feat_hidden2
        params.modified_latent_dim=10
        self.modified_latent_dim = params.modified_latent_dim
        ...
        self.decoder.add_module('decoder_L0', full_block(self.modified_latent_dim, input_dim, params.p_drop))

        # introduce this to map both vgae and vae latent spaces to the same one
        self.fc = nn.Sequential()
        self.fc.add_module('fully_connected_L3', full_block(self.latent_dim, self.modified_latent_dim, 0))
        ...
        # DEC cluster layer
        self.cluster_layer = Parameter(torch.Tensor(params.dec_cluster_n, self.modified_latent_dim))
    ...
    def forward(self, x, adj):
        ...
        # map the combined latent space to a new latent space which decodes for both vae and vgae
        z = self.fc(z)
