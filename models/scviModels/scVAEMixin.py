...
from scvi.model.base._log_likelihood import compute_elbo, compute_reconstruction_error
...
class scVAEMixin:
    ...
    def get_latent_representation_with_grad(
        self,
        adata: Optional[AnnData] = None,
        indices: Optional[Sequence[int]] = None,
        give_mean: bool = True,
        ...
        for tensors in scdl:
            ...
            latent_qzv += [qz.scale.square().cpu()]
        return (
            (torch.cat(latent_qzm), torch.cat(latent_qzv))
            if return_dist
            else torch.cat(latent)
        )